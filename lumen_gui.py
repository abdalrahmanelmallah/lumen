...
class TabState:
    def __init__(self, code="", filepath=None):
        self.code = code
...
class LumenGUI(tk.Tk):
    ...
    def _on_key(self,event=None):
        self._highlight()
        self.linenumbers.redraw()
        if not self.tabs[self.current_tab].modified:
    ...
    def _open_path(self,path):
        ...
        if not cur.filepath and not cur.modified and cur.code.strip()==DEFAULT_CODE.strip():
            cur.code=content;cur.filepath=path;cur.modified=False
            self._load_tab(self.current_tab)
        else:
            self.new_tab(content,path);self.tabs[self.current_tab].modified=False;self._rebuild_tabbar()
    ...
    def save_file(self):
        ...
        try:
            with open(tab.filepath,"w",encoding="utf-8") as f:f.write(tab.code)
            tab.modified=False
            self._rebuild_tabbar()
