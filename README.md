...
[![CI](https://github.com/abdalrahmanelmallah/lumen/actions/workflows/ci.yml/badge.svg)](https://github.com/abdalrahmanelmallah/lumen/actions/workflows/ci.yml)
[![Latest release](https://img.shields.io/github/v/release/abdalrahmanelmallah/lumen)](https://github.com/abdalrahmanelmallah/lumen/releases)
[![License](https://img.shields.io/github/license/abdalrahmanelmallah/lumen)](LICENSE)
...
- lists — list/array literals (`[1, 2, 3]`) and indexing (`list[0]`) are built into the language; `import "lists"` adds `push`, `pop`, `map`, `sort`, `unique`, `flatten`, `chunk`, `zip`, and more. Full reference: [docs/lists-library.md](docs/lists-library.md)
- `strings` — string manipulation (`upper`, `split`, `contains`, `replace`, `capitalizeWords`, and more). Full reference: [docs/strings-library.md](docs/strings-library.md)
- `file` — read/write text files, search inside them (`readFile`, `writeFile`, `appendFile`, `searchInFile`, `searchInFiles`). Full reference: [docs/file-library.md](docs/file-library.md)
- `json` — parse and write JSON (`jsonParse`, `jsonStringify`, `jsonStringifyPretty`). Full reference: [docs/json-library.md](docs/json-library.md)
- `re` — regular expressions (`reTest`, `reMatch`, `reFindAll`, `reReplace`, and more). Full reference: [docs/re-library.md](docs/re-library.md)
- `datetime` — date/time formatting and arithmetic (`now`, `formatTime`, `parseTime`, `addDays`, and more). Full reference: [docs/datetime-library.md](docs/datetime-library.md)
- `format` — number/text formatting (`toFixed`, `padStart`, `padEnd`, `withCommas`, `zeroPad`). Full reference: [docs/format-library.md](docs/format-library.md)
- `collections` — `Stack`, `Queue`, and `Set` classes. Full reference: [docs/collections-library.md](docs/collections-library.md)
- `test` — assertion helpers (`assertEqual`, `assertTrue`, `assertClose`, and more) for writing tests in Lumen. Full reference: [docs/test-library.md](docs/test-library.md)
- `encrypt` — real keyed encryption plus classic educational ciphers (Caesar, ROT13, Vigenère, and more). Full reference: [docs/encrypt-library.md](docs/encrypt-library.md)
...
- `system` — desktop integration (notifications, dialogs, wallpaper, opening URLs/files). Full reference: [docs/system-library.md](docs/system-library.md)
- `os` — environment variables, platform detection, shell commands, paths. Full reference: [docs/os-library.md](docs/os-library.md)
- `sys` — `clock()`, `sleep(s)`
...
`tests/` exercises the interpreter directly (`test_core.py`), each
library through small `.lu` snippets (`test_libraries.py`), and every
file in `examples/` end-to-end (`test_examples.py`). CI runs this same
suite on Linux, macOS, and Windows on every push and pull request — see
[.github/workflows/ci.yml](.github/workflows/ci.yml).

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) — adding a library is usually the
easiest kind of contribution, since most are a single `.lu` file that
doesn't touch the interpreter at all.
